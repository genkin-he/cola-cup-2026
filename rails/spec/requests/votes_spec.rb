...
RSpec.describe "Votes", type: :request do
  ...
  describe "POST /matches/:id/vote" do
    ...
    it "records a vote with the server-determined stake for the stage" do
      sign_in user
      match = create(:match, :knockout) # r16 -> stake 2.0
      post match_vote_path(match), params: { pick: "home", stake: "999" }, as: :turbo_stream

      expect(response).to have_http_status(:ok)
      vote = Vote.sole
      expect(vote).to have_attributes(user_id: user.id, match_id: match.id, pick: "home", stake: 2.0)
    end

    it "upserts in place when the pick changes" do
      sign_in user
      match = create(:match)
      post match_vote_path(match), params: { pick: "home" }, as: :turbo_stream
      expect { post match_vote_path(match), params: { pick: "away" }, as: :turbo_stream }
