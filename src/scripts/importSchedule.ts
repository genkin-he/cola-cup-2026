...
async function main(): Promise<void> {
  const teams = await fetchJson<RawTeam[]>(`${BASE}/worldcup.teams.json`);
  const schedule = await fetchJson<{ name: string; matches: RawMatch[] }>(
    `${BASE}/worldcup.json`,
  );
  ...
  const byStage = db
    .prepare("SELECT stage, COUNT(*) AS count FROM matches GROUP BY stage")
    .all();
  console.log(
    `Imported ${teams.length} teams, ${schedule.matches.length} matches.`,
  );
